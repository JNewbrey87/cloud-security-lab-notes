...
The records in this ledger that predate 2026-09-11 were produced on CPU; the machine's GPU inference path had silently regressed and was not restored until that date. That one sentence is the only thing distinguishing the two eras in the chain, because I chose not to add code or a second file to encode it. Worth saying plainly rather than hiding: the chain is therefore not self-describing on this point, which is a mild tension with the framework's own principle that evidence should stand alone. I would rather name the tension than paper over it. The infrastructure itself is sound; a full two-agent crew run completes in a few minutes once inference is actually on the GPU.
